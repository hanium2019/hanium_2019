...
namespace Leap.Unity.Interaction {
  ...
  public class InteractionBehaviour : MonoBehaviour, IInteractionBehaviour {
    ...
    public const float MAX_ANGULAR_VELOCITY = 0.1F;
    ...
    public void BeginContact(List<InteractionController> controllers) {
      ...
      if (handWatcher.windowState != 0 && !contacting) {
        contacting = true;
        LeanTween.alpha(this.gameObject , 0.85f,0.1f); 
        Debug.Log("Contact");
      }
    }
    ...
    public void EndContact(List<InteractionController> controllers) {
      ...
      CancelInvoke();
      StopWindow();
      handWatcher.ParticleControl(0, false);
      contacting = false;
      touchPlay = false;
      if(handWatcher.windowState != 0 && _graspingControllers.Count == 0)
        LeanTween.alpha(this.gameObject, 0.7f,0.1f);
      
    }

    bool isFaded = true;
    bool touchPlay = false;
    public void StayContacted(List<InteractionController> controllers) {
      OnContactStay(); // 저 좌표축 잡아봐요
      if (handWatcher.windowState != 0) {
        if (controllers.Count == 1 && this.gameObject.tag == "Window") {                             //해당 윈도우를 살짝이라도 잡고 있는 컨트롤러가 1개 (한손 잡기)
          if (controllers[0] == leftInteractionHand) {                //왼손으로 잡은 상태
            if (handWatcher.leftHandState == 2) {                          //쫙 핀 상태
              if (!touchPlay) {
                touchPlay = true;
                Invoke("PlayWindow", 0.6f);
                handWatcher.ParticleControl(1, false);
              }
            }
            else {
              handWatcher.ParticleControl(0, false);
    ...
    void PlayWindow() {
      handWatcher.ParticleControl(0, false);
      captureManager.FindTargetHandle(this.gameObject);
      captureManager.RefreshStart();
    }

    void StopWindow() {
      captureManager.RefreshStop();
    }
    ...
    public void EndGrasp(List<InteractionController> controllers) {
      ...
      if (_graspingControllers.Count == 0) {
        ...
        captureManager.RefreshStop();
        grabed = false;
        handWatcher.ParticleControl(0, false);                //Stop left Particle
        handWatcher.ParticleControl(0, true);                //Stop left Particle
        if (handWatcher.windowState != 0 && !twoHanded) {
          //LeanTween.scale(this.gameObject, new Vector3(x, y, z), 0.15f);
          LeanTween.alpha(this.gameObject ,0.7f,0.1f);
        }
        twoHanded = false;
    ...
    public Vector3 resize_right, resize_left;
    public bool refresh = false;
    public bool resize = false;
    public bool grabed = false;


    // window object look camera
    public float getAngle(Vector3 vec)
    {
        float degree;

        // set standard vector and set window's relative vector from camera
        Vector3 standardVec = new Vector3(mainCamera.transform.position.x, 0, mainCamera.transform.position.z + 1);
        Vector3 relativeVec = new Vector3(vec.x - mainCamera.transform.position.x, 0, vec.z - mainCamera.transform.position.z);
        degree = Mathf.Acos(Vector3.Dot(standardVec, relativeVec) / Vector3.Magnitude(standardVec) / Vector3.Magnitude(relativeVec));
    ...
    public void StayGrasped(List<InteractionController> controllers) {
      if (moveObjectWhenGrasped) {
        ...
        if (controllers.Count == 1) {                             //해당 윈도우를 살짝이라도 잡고 있는 컨트롤러가 1개 (한손 잡기)
          if (twoHanded) {
            twoHanded = false;
            handWatcher.ParticleControl(0, false); 
            handWatcher.ParticleControl(0, true);
          }
          float diffdegree;

          diffdegree = getAngle(this.transform.position);
          newRotation = Quaternion.Euler(this.transform.rotation.x, diffdegree, this.transform.rotation.z);
          //newRotation = Quaternion.LookRotation(Vector3.zero);

          if (controllers[0] == leftInteractionHand) {                //왼손으로 잡은 상태
            if (handWatcher.leftHandState == 0) {                           //왼손으로 꽉 쥔 상태
              if (!grabed && handWatcher.windowState != 0) {                       //처음 꽉 쥔거라면 확대, 불투명 시각 피드백.
                grabed = true;
                x = this.gameObject.transform.localScale.x;
                y = this.gameObject.transform.localScale.y;
                z = this.gameObject.transform.localScale.z;
                handWatcher.ParticleControl(3, false);                //Play left Particle
                //LeanTween.scale(this.gameObject, new Vector3(x, y, z)*1.07f, 0.15f);
                LeanTween.alpha(this.gameObject , 1f,0.1f);
              }
              if (handWatcher.windowState == 2)
                fixedUpdateGraspedMovement(new Pose(origPosition, origRotation),      //이동
                                    new Pose(newPosition, newRotation),
                                    controllers);
              else if (handWatcher.windowState == 1)
                fixedUpdateGraspedMovement(new Pose(origPosition, origRotation),      //이동
                                    new Pose(newPosition, origRotation),
                                    controllers);
                                                                                    //Particle System 이용해 잡은 상태임을 표시 (반짝반짝)
            }
            else {                                                          //왼손으로 집고 있거나 다 핀 상태
              if (grabed && handWatcher.windowState != 0) {                          //이전까지 쥐고 있었던거라면 축소, 반투명 시각 피드백
                grabed = false;
                handWatcher.ParticleControl(0, false);                //Stop left Particle
                //LeanTween.scale(this.gameObject, new Vector3(x, y, z), 0.15f);
                LeanTween.alpha(this.gameObject , 0.7f,0.1f);
          ...
          if (controllers[0] == rightInteractionHand) {                //오른손으로 잡은 상태
            if (handWatcher.rightHandState == 0) {                           //오른손으로 꽉 쥔 상태
              if (!grabed && handWatcher.windowState != 0) {                                                       //처음 꽉 쥔거라면 확대, 불투명 시각 피드백.
                grabed = true;
                x = this.gameObject.transform.localScale.x;
                y = this.gameObject.transform.localScale.y;
                z = this.gameObject.transform.localScale.z;
                handWatcher.ParticleControl(3, true);                //Play right Particle
                //LeanTween.scale(this.gameObject, new Vector3(x, y, z)*1.07f, 0.15f);
                LeanTween.alpha(this.gameObject , 1f,0.1f);
              }
              if (handWatcher.windowState == 2)
            ...
            else {                                                          //오른손으로 집고 있거나 다 핀 상태
              if (grabed && handWatcher.windowState != 0) {                                                        //이전까지 쥐고 있었던거라면 축소, 반투명 시각 피드백
                grabed = false;
                handWatcher.ParticleControl(0, true);                //Stop right Particle
                //LeanTween.scale(this.gameObject, new Vector3(x, y, z), 0.15f);
                LeanTween.alpha(this.gameObject , 0.7f,0.1f);
              }
            }
        ...
        } else if (controllers.Count == 2) {                             //양손으로 잡고 있는 상태
          if (!twoHanded) {                                                     //양손으로 처음 잡은거면 현재 사이즈 저장
            twoHanded = true;
            x = this.transform.localScale.x;
            y = this.transform.localScale.y;
            z = this.transform.localScale.z;
            handWatcher.ParticleControl(2, true);
            handWatcher.ParticleControl(2, false);
          }
          fixedUpdateGraspedMovement(new Pose(origPosition, origRotation),
                                   new Pose(newPosition, newRotation),
                                   controllers);
          this.transform.localScale = new Vector3(x, y, z)*(0.4f+3f*Vector3.Distance(rightThumb.transform.position, leftThumb.transform.position));  //사이즈 조정
    ...
    private bool _isPositionLocked = true;
